...
function mPriv(){
    ...
    // Animation
    page.style.transition = "opacity 0.5s ease, transform 0.7s ease";
...
if("serviceWorker" in navigator && !localStorage.getItem("HM-Date")){
    navigator.serviceWorker.register("js/serviceWorker.js")
    .then(()=>{
        console.log("Service Worker berhasil");
    })
}else if(lastData.date === new Date().toLocaleDateString("id-ID")){
    alert("Selamat Datang kembali");
    install.classList.add("hidden");
    localStorage.setItem("HM-Date", new Date().toLocaleDateString("id-ID"));
}else{
    let now = new Date().toLocaleDateString("id-ID").split("/");
    let last = lastData.date.split("/");
    if(now[0] - last[0] > 3){
        alert(`Haii, ingat ${now[0] - last[0]} hari yang lalu ngak? saat terakhir kali kamu buka akuu!`);
    }
    install.classList.add("hidden");
    localStorage.setItem("HM-Date", new Date().toLocaleDateString("id-ID"));
};
...
function pasang(){
    if(installPrompt){
        ...
        localStorage.setItem("HM-Date", new Date().toLocaleDateString("id-ID"));
        document.getElementById("btnI").remove();
        let SI = document.createElement("p");
        SI.textContent = "Installing.....";
        SI.style.color = "blue";
        SI.style.fontSize = "small";
        install.append(SI);
        localStorage.setItem("HM-Date", new Date().toLocaleDateString("id-ID"));
    }
};
...
function swipe(e){
    if(e.type == "touchstart"){
        ...
        if((startTouch - endTouch) > 120){
            mPriv();
        }else if((startTouch - endTouch) < -120){
            mNext();
        }
    }
}
page.addEventListener("touchstart", swipe);
pageTr.addEventListener("touchstart", swipe);
page.addEventListener("touchend", swipe);
pageTr.addEventListener("touchend", swipe);